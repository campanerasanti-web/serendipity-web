        ...
        >
            ...
            <div className={`rounded-[18px] border overflow-hidden ${T}`} style={{ borderColor: isDark ? '#3f3f46' : 'rgba(228,228,235,0.9)', boxShadow: isDark ? '0 28px 70px rgba(0,0,0,0.65)' : '0 28px 70px rgba(0,0,0,0.13)' }}>
                ...
                <div className={`flex ${T} h-[280px] sm:h-[420px] lg:h-[520px]`} style={{ background: bg0 }}>
                    ...
                    {/* Sidebar */}
                    <div className="hidden sm:flex flex-shrink-0 flex-col" style={{ width: '176px', background: sidebarBg, borderRight: `1px solid ${sidebarBdr}` }}>
                        {/* Logo */}
                    ...
                    <div className={`flex-1 flex flex-col overflow-hidden ${T}`} style={{ background: bg1 }}>
                        ...
                        <div className="flex-1 p-3 sm:p-4 overflow-y-auto overflow-x-hidden flex flex-col gap-3 hide-scrollbar">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                {[
                                    { title: 'Matriz de Ritmos', sub: lang === 'es' ? 'Progreso de producción' : 'Production', value: '108.000 SF', meta: 'Meta: 150.000 SF', icon: Activity, color: '#3b82f6', pct: 72, desc: lang === 'es' ? 'Días de sol.' : 'Running steady.' },
                                    { title: 'Fondo de Paz',     sub: lang === 'es' ? 'Amortización Prara Asia' : 'Amortization', value: '$15.000', meta: 'Meta: $40.000', icon: Shield, color: '#22c55e', pct: 37, desc: lang === 'es' ? 'Flujo activo.' : 'Active flow.' },
                                ].map((card, i) => (
                                    <div key={i} className={`rounded-xl border p-3 flex flex-col justify-between ${T}`} style={{ background: cardBg, borderColor: cardBdr, minHeight: '90px' }}>
